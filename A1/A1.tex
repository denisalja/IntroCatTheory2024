...
\newcommand{\geoff}[1]{\todo[color=red!20]{{\bf Geoff:} #1}}
\newcommand{\deni}[1]{\todo[color=blue!25]{{\bf Deni:} #1}}
\newcommand{\biggeoff}[1]{\todo[inline,color=red!20]{{\bf Geoff:} #1}}
...
\newtheorem*{negg}{Non-Example}
\newtheorem*{eggs}{Examples}
...
\newtheorem{excs}[thm]{Exercise(s)}
\newtheorem{egs}[thm]{Examples}
\newtheorem{prob}[thm]{Problem}
\newtheorem{conj}[thm]{Conjecture}
\newtheorem{ques}[thm]{Question}
%Making crossed arrows available as marking in tikcd
...
%Double Cat Shortcuts
%single tikzcd pro-arrow: \artkz{domain}{name}{codomain}
\newcommand*\prartkz[3]{{\begin{tikzcd}[ampersand replacement=\&] 
{ #1 } \rar["{#2}", "\bullet" marking] \&  { #3 } \end{tikzcd}}}
%a square: \sq{4 objects}{4 arrows}{cell}
\newcommand*\sq[9]{{\begin{tikzcd}[column sep = large, row sep = large, ampersand replacement=\&] 
{#1} \rar["\bullet"{marking}, "{#7}", 
{name=top, below}] \dar["{#5}"'] 
\& {#2} \dar["{#8}"] \ 
{#3} \rar["\bullet"{marking},"{#6}"', 
{name=bot, above}] 
\& {#4}
\ar[Rightarrow, from=top, to=bot, shorten >= 8 , shorten <= 8, "{#9}" ] \end{tikzcd}}}
%double categories
\newcommand{\dbSet}{\mathbb{S}\mathrm{et}}
%big roomy quotient
\newcommand{\nat}[6][large]{%
 \begin{tikzcd}
...
\end{tikzpicture}}
%pushout comand: \arrow[dl, phantom, "\usebox\urpushout" , very near start, color=black]
%vertical pullback
\newsavebox{\vpullback}
\sbox\vpullback{%
\begin{tikzpicture}%
\draw (0ex,1ex) -- (1ex, 0ex);%
\draw (1ex, 0ex) -- (2ex,1ex);%
\end{tikzpicture}}
%vert. pullback comand: \arrow[dd, phantom, "\usebox\vpullback" , very near start, color=black]
%vertical pushout
\newsavebox{\vpushout}
\sbox\vpushout{%
\begin{tikzpicture}%
\draw (0ex,1ex) -- (1ex, 1ex);%
\draw (1ex, 1ex) -- (2ex,1ex);%
\end{tikzpicture}}
%vert. pushout comand: \arrow[uu, phantom, "\usebox\vpushout" , very near start, color=black]
\newcommand{\te}[1]{\text{#1}}
